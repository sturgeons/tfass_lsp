Ext.define('LDPlatformModule.controller.Controller', {
    ...
    init: function() {// Cotroller的业务处理
	    this.control({
	                    ...
	                    gridPanel.getStore().filter({
	                        filterFn: function(item) {
	                            return !field.getValue() || 
	                                ...
	                                ( item.get("SLICE_NO") && item.get("SLICE_NO").indexOf(field.getValue()) > -1); 
	                        }
	                    });
	                }
	            }
	    	...
	    	'formcomponent combobox[name=QADNO]':{
	    		change: function(sel, val) {
    				Ext.getCmp('fabric_no').setValue(null);
    				Ext.getCmp('fabric_no').getStore(). reload({
    					params: {QADNO: val}
    				});
	            }
	        },
