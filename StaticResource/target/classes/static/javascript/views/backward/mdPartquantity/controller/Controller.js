Ext.define('LDPlatformModule.controller.Controller', {
    ...
    init: function() {// Cotroller的业务处理
	    this.control({
	    	...
	    	'button[action=add]':{
	    		click: function(){
	    			Ext.getCmp('reason_comp').setVisible(false);
	    			Ext.getCmp('reason_comp').allowBlank = true;
	        ...
	        'button[action=modify]': {
	        	click: function(){
	    			 Ext.getCmp('reason_comp').setVisible(true);
		    			Ext.getCmp('reason_comp').allowBlank = false;
	        ...
	        'formcomponent combobox[name=LINE]': {
	        	change: function(sel, val) {
	        		var comp = this.getFormPanel().down('combobox[name=OP]');
	        	...
	        	click: function(){
	                ...
	                if (form.isValid()) {
	        			...
	        			if(record) {// 编辑数据
	        				...
	        				record.data.EM_ID = Ext.getDom('em_id').value;
	        			} else {// 新增数据
